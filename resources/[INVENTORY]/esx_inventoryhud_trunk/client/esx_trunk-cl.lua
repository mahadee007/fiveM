...
RegisterCommand("inventeryVeh", 
function()
    openInventory()      ----openmenuvehicle()
end
)
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
				...
				local weightTrunk = 0
                if model == "MOONBEAM" then     -----รถแจก40โล
                  weightTrunk = 40000
                else
                  weightTrunk = Config.VehicleLimit[class]
                end 
				--OpenCoffreInventoryMenu(GetVehicleNumberPlateText, Config.VehicleLimitModel[Vehicle_Model])
                OpenCoffreInventoryMenu(GetVehicleNumberPlateText(vehFront), weightTrunk, myVeh)
              end
            else
...
Citizen.CreateThread(
  function()
    while true do
      Wait(0)
      if IsControlJustReleased(0, 182) and (GetGameTimer() - GUI.Time) > 1000 then
        openmenuvehicle()
        GUI.Time = GetGameTimer()
      end
    end
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    ...
    function(inventory)
      ...
      data = {plate = plate, max = max, myVeh = myVeh, text = text,weight = inventory.weight}
...
--[[function OpenCoffreInventoryMenu(plate, max)
    ...
    ESX.TriggerServerCallback("esx_trunk:getInventory",function(inventory)
