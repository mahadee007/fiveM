...
function refreshPropertyInventory()
    ESX.TriggerServerCallback(
        "meeta_house:getPropertyInventory",
        function(inventory)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("meeta_house:putItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("meeta_house:getItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
        end
