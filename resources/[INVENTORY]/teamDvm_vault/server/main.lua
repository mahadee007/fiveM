...
RegisterServerEvent('teamDvm_vault:getItem')
AddEventHandler('teamDvm_vault:getItem', function(--[[owner,--]] job, type, item, count)
...
RegisterServerEvent('teamDvm_vault:putItem')
AddEventHandler('teamDvm_vault:putItem', function(--[[owner,--]] job, type, item, count)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
	local xPlayerOwner = ESX.GetPlayerFromIdentifier(xPlayer.identifier)
...
PerformHttpRequest(ServerUrl.."/check.php?sc=teamDvm_vault", function(err, rText, headers)
	print("\n\n[^9teamDvm_vault^0]^1!^0[^2Thanks you! Supporpting this script^0]\n")
	if rText then
...
ESX.RegisterServerCallback('teamDvm_vault:getVaultInventory', function(source, cb, item, refresh)
	-- local xPlayer    = ESX.GetPlayerFromIdentifier(owner)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xItem
	if item.needItemLicense ~= '' and item.needItemLicense ~= nil then
		print('xItem: assign')
		xItem = xPlayer.getInventoryItem(item.needItemLicense)
	else
		print('xItem: not assign')
		xItem = nil
	end
	...
	if not refresh and (item.needItemLicense ~= '' and item.needItemLicense ~= nil) and xItem ~= nil and xItem.count < 1 then
		print('cb')
		-- if  then
		cb(false)
		-- end
	...
	elseif not item.InfiniteLicense and not refresh and xItem ~= nil and xItem.count > 0 then
		print('check: InfiniteLicense')
		-- if not item.InfiniteLicense then
			-- print('not item.InfiniteLicense')
