...
function OpenVaultInventoryMenu(data)
	if data.job == ESX.PlayerData.job.name or data.job == 'vault' then
		--print('need license = '..data.needItemLicense)
		-- if data.InfiniteLicense then
		-- 	print(' license infinite = true')
		-- else
		-- 	print(' license infinite = false')
		-- end
		vaultType = data
	...
	while true do
		...
		for k,v in pairs(Config.Vault) do
			...
			if dist < 2 then
				...
				DrawTD3DText(v.coords.x, v.coords.y, v.coords.z +1.0, '[~b~E~w~]'..k)
...
function DrawTD3DText (x,y,z, text)
    ...
    DrawRect(_x,_y+0.0160, 0.030+ factor, 0.03, 55,55,55, 170)
end