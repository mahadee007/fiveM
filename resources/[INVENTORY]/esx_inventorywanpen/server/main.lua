...
ESX.RegisterServerCallback("esx_inventorywanpen:getPlayerInventory", function(source, cb, target)
...
RegisterServerEvent("esx_inventorywanpen:tradePlayerItem")
AddEventHandler("esx_inventorywanpen:tradePlayerItem", function(from, target, type, itemName, itemCount)
		local _source = from
...
RegisterCommand("openinventory", function(source, args, rawCommand)
		if IsPlayerAceAllowed(source, "inventory.openinventory") then
			...
			if targetXPlayer ~= nil then
				TriggerClientEvent("esx_inventorywanpen:openPlayerInventory", source, target, targetXPlayer.name)
			else
