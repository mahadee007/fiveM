...
RegisterNetEvent("scotty-policealert:sendLocation")
AddEventHandler("scotty-policealert:sendLocation", function(pos)
...
Citizen.CreateThread(function()
	while true do
		...
		if ispressed(1, Config["base_key"]) and num and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			TriggerServerEvent("scotty-policealert:getLocation", num)
			Citizen.Wait(1000)
		end
	end
end)
...
RegisterNetEvent('scotty-policealert:alertArea')
AddEventHandler('scotty-policealert:alertArea', function(pos)
	Citizen.CreateThread(function()
		SendNUIMessage({
			type = 'playsound',
		})
...
AddEventHandler("scotty-policealert:alertNet", function(event_type)
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
	TriggerServerEvent("scotty-policealert:defaultAlert", event_type, gender, getStreetName(), pos)
end)

RegisterNetEvent("scotty-policealert:getalertNet")
AddEventHandler("scotty-policealert:getalertNet", function(event_type)
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
	TriggerServerEvent("scotty-policealert:defaultAlert", event_type, gender, getStreetName(), pos)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if Config["alert_section"]["carjacking"] and IsPedJacking(ped) then
			...
			if seat == ped then
				local pos = GetEntityCoords(ped)
				TriggerServerEvent("scotty-policealert:defaultAlert", "carjacking", gender, getStreetName(), pos)
			end
			Citizen.Wait(Config["duration"] * 1000)
		elseif Config["alert_section"]["melee"] and IsPedInMeleeCombat(ped) then
			Citizen.Wait(3000)
			local pos = GetEntityCoords(ped)
			TriggerServerEvent("scotty-policealert:defaultAlert", "melee", gender, getStreetName(), pos)
			Citizen.Wait(Config["duration"] * 1000)
		elseif Config["alert_section"]["gunshot"] and IsPedShooting(ped) and not IsPedCurrentWeaponSilenced(ped) then
			Citizen.Wait(3000)
			local pos = GetEntityCoords(ped)
			TriggerServerEvent("scotty-policealert:defaultAlert", "gunshot", gender, getStreetName(), pos)
			Citizen.Wait(Config["duration"] * 1000)
		end
	end
end)