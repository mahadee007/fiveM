...
AddEventHandler('esx_aiTaxi:callTaxi', function(coords)
	if customer then
		ESX.ShowHelpNotification('Taxi กำลังอยู่ระหว่างทาง')
	else
		customer = coords
		-- get best spawnpoint
		...
		if customer ~= nil then
			...
			if vehicle == globalTaxi then
				...
				if not DoesBlipExist(waypoint) and not onTour then
					ESX.ShowHelpNotification('คุณจะไปที่ไหน ?')
					Citizen.Wait(2000)
				else
			...
			if customer ~= nil then
				...
				if distanceMeTaxi <= 40 then
					if not parkingDone then
						...
						TriggerEvent('esx:showNotification', 'Taxi มาถึงแล้ว !!')
					end
...
function atTarget(cancel)
	...
	if cancel then
		...
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		if vehicle ~= globalTaxi then
			TriggerEvent('esx:showNotification', 'Taxi ได้ถูกยกเลิกแล้ว ')
			cancelTaxi = true
		else
			TriggerEvent('esx:showNotification', 'ไม่สามารถยกเลิก Taxi ได้ !!')
			return
		end
	end
...
function drive(x, y , z, delete, status)
	if status == 'start' then
		...
		ESX.ShowHelpNotification('Taxi กำลังมารับคุณ...')
	elseif status == 'end' then
		ESX.ShowHelpNotification('ขอบคุณที่ใช้บริการ Taxi !! ')
	end
