...
local Positions = {
	...
	['Sell'] = { ['hint'] = '[E] Sell', ['x'] = 259.26, ['y'] = -1801.87, ['z'] = 27.12 },
	...
	if OnGoingHuntSession then
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				local Animal = CreatePed(5, GetHashKey('a_c_chimp'), value.x, value.y, value.z, 0.0, true, false)
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('chimp - Animal')
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('You slaughtered the animal and recieved an monkey of ' ..AnimalWeight.. 'kg')

	TriggerServerEvent('esx-killmonkey:reward', AnimalWeight)

	DeleteEntity(AnimalId)
end

function SellItems()
	TriggerServerEvent('esx-killmonkey:sell')
end
