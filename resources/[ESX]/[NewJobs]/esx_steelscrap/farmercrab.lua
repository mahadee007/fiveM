	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			if not isProcessing then
				...
				DisplayHelpText('Press [~g~E~s~] to Process ~b~ Scrap~s~')
			end
...
function Process()
	...
	TriggerServerEvent('esx_steelscrap:process')
	...
	TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, false)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), config.zones.process.coords, false) > 4 then
			TriggerServerEvent('esx_steelscrap:cancelProcessing')
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
					--if not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR" then
						...
						DisplayHelpText('Press [~g~E~s~] to ~b~ Cutting~s~')
					--end
					...
					--if IsControlJustReleased(0, 38) and not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR" then
						ESX.TriggerServerCallback('esx_steelscrap:haveItem', function(haveItem)
							if haveItem then
								isPickingUp = true
						
									ESX.TriggerServerCallback('esx_steelscrap:canPickUp', function(canPickUp)
						...
						--
											...
											TriggerServerEvent('esx_steelscrap:pickedUp')
											else
								...
								ESX.ShowNotification("You need to buy welding_gun")
							end
...
function SpawnProps()
	while spawnedProps < 25 do
		...
		local Coords = GeneratePropCoords()
			...
			ESX.Game.SpawnLocalObject('prop_rub_carwreck_15', Coords, function(obj)
