...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.GoldField.coords, true) < 40 then
			SpawnedGoldds()
			Citizen.Wait(500)
		else
			Citizen.Wait(500)
...
function SpawnedGoldds()
	while spawnedGolds < 40 do
		Citizen.Wait(0)
		local goldCoords = GenerateGoldCoords()

		ESX.Game.SpawnLocalObject('csx_coastsmalrock_02_', goldCoords, function(obj)
...
function ValidateGoldCoord(plantCoord)
	if spawnedGolds > 0 then
		...
		if GetDistanceBetweenCoords(plantCoord, Config.CircleZones.GoldField.coords, false) > 40 then
			validate = false
		end
