...
AddEventHandler('audio:success', function()
...
AddEventHandler('audio:unsuccess', function()
	--Trigger the interaction sound
...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('scotty:craftGetItems1', function(data)
		SendNUIMessage({
			type = 'load-item',
			data = data
		})
...
RegisterNUICallback("Craft", function(data, cb)
	CraftItem1(data.cate_id, data.item_id, data.amount)
end)
...
function CraftItem1(cate_id, item_id, amount)
	amount = amount or 1
	...
	if to_remove <= 0 then
		...
		Citizen.CreateThread(function()
			...
			if not IsEntityDead(PlayerPedId()) then
				...
				TriggerServerEvent('scotty:craftItem1', cate_id, item_id, amount)
			...
			for k,v in ipairs(Config["craft_table"]) do
				if v.position ~= nil then
					...
					if dist <= (v.max_distance or 2.0) and craft_percent == -1 and not menu_active then
						...
						AddTextComponentString(" ~INPUT_CONTEXT~ ~b~CRAFTING")
