...
AddEventHandler('esx_jobCounter:get', function()
	...
	TriggerClientEvent('esx_ambulancejob:set', source, counted)
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'oxygen_mask' and itemName ~= 'fixkit') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterUsableItem('medikit', function(source)
	...
	TriggerClientEvent('chatMessage', -1, "^1ระบบ : ", {255, 153, 51},'^0คุณหมอ^2' .. xPlayer.name .. ' ^0ได้ใช้อุปกรณ์ทางแพทย์ ชุบชีวิตตัวเองแล้ว')
	else
...
RegisterServerEvent('esx_ambulancejob:getDeathStatus')
AddEventHandler('esx_ambulancejob:getDeathStatus', function()
	local _source    = source
