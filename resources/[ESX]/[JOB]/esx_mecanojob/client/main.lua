...
function OpenMobileMecanoActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_mecano_actions',
    {
      title    = ('เมนูช่าง'),
      elements = {
        {label = ('ปลดล็อกรถ'),     value = 'hijack_vehicle'},
        {label = ('ซ่อมรถ'),       value = 'fix_vehicle'},
        {label = ('เช็ดรถ'),      value = 'clean_vehicle'},
        {label = ('เก็บรถ'),     value = 'del_vehicle'},
        {label = _U('flat_bed'),       value = 'dep_vehicle'},
        {label = ('วางสิ่งของ'), value = 'object_spawner'}
      }
    },
    function(data, menu)
        ...
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
          ...
          if IsPedInAnyVehicle(playerPed, false) then
			...
			Citizen.CreateThread(function()
				...
				Citizen.Wait(20000) -- Right before this <---------
