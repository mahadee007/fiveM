	...
	while true do
		...
		if DoesEntityExist(PlayerPed) and not IsEntityDead(PlayerPed) and (PlayerData.job ~= nil and PlayerData.job.name == 'police') then
            if not IsPauseMenuActive() then 
                ...
                if IsControlJustPressed(0, Config.Key['F4']) and (PlayerData.job ~= nil and PlayerData.job.name == 'police') and not IsHolster then -- Left Alt
                ...
                if IsControlJustReleased(0, Config.Key['F4']) and (PlayerData.job ~= nil and PlayerData.job.name == 'police') and not IsHolster then
                    ClearPedTasks(PlayerPed)
                end
