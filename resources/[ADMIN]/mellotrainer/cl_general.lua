...
Citizen.CreateThread( function()
	while true do
		...
		if ( IsControlJustReleased( 0, 208 ) or IsDisabledControlJustReleased( 0, 208 ) ) and GetLastInputMethod( 0 ) and not blockinput and ((settings["adminOnlyTrainer"] == true and adminStatus == true) or settings["adminOnlyTrainer"] == false) then -- f3
			teleportToWaypoint()
		end

		if ( IsControlJustReleased( 0, 56 ) or IsDisabledControlJustReleased( 0, 56 ) ) and GetLastInputMethod( 0 ) and ((settings["adminOnlyNoclip"] == true and adminStatus == true) or settings["adminOnlyNoclip"] == false) then
			toggleNoClipMode()
		end
