--TeamMenu version 0.9.7

TeamMenu = { }

-- Options
TeamMenu.debug = false
...
local function debugPrint(text)
    if TeamMenu.debug then
        Citizen.Trace('[TeamMenu] '..tostring(text))
    end
end
...
function TeamMenu.CreateMenu(id, title)
    -- Default settings
    menus[id] = { }
...
function TeamMenu.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        TeamMenu.CreateMenu(id, menus[parent].title)
...
function TeamMenu.CurrentMenu()
    return currentMenu
end


function TeamMenu.OpenMenu(id)
    if id and menus[id] then
        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
        setMenuVisible(id, true)
        debugPrint(tostring(id)..' menu opened')
		TriggerEvent('14104ee0-314f-418e-938a-45813e7cb9a8')
    else
...
function TeamMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end


function TeamMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function TeamMenu.CloseMenu()
    if menus[currentMenu] then
        if menus[currentMenu].aboutToBeClosed then
            ...
            currentMenu = nil
            currentKey = nil
			TriggerEvent('448d1dba-2d48-4bc4-a45e-dde0ab0394f7')
        else
...
function TeamMenu.Button(text, subText)
...
function TeamMenu.MenuButton(text, id, subText)
    if menus[id] then
        if TeamMenu.Button(text, subText) then
            setMenuVisible(currentMenu, false)
...
function TeamMenu.CheckBox(text, bool, callback)
    local checked = 'Off'
    if bool then
        checked = 'On'
    end

    if TeamMenu.Button(text, checked) then
        bool = not bool
...
function TeamMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if TeamMenu.Button(text, selectedItem) then
...
function TeamMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end


function TeamMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end


function TeamMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end


function TeamMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end


function TeamMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleColor.a })
end
 
 
function TeamMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleBackgroundColor.a })
end


function TeamMenu.SetSubTitle(id, text)
    setMenuProperty(id, 'subTitle', string.upper(text))
end


function TeamMenu.SetMenuBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'menuBackgroundColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuBackgroundColor.a })
end


function TeamMenu.SetMenuTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuTextColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuTextColor.a })
end

function TeamMenu.SetMenuSubTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuSubTextColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuSubTextColor.a })
end

function TeamMenu.SetMenuFocusColor(id, r, g, b, a)
    setMenuProperty(id, 'menuFocusColor', { ['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuFocusColor.a })
end


function TeamMenu.SetMenuButtonPressedSound(id, name, set)
